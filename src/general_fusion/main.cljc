(ns general-fusion.main
  "Kotodama WASM entrypoint for the General_fusion clean-room actor (L4) — Clojure port.
  ...
  facts (`general_fusion.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "general_fusion")
(def tier "L4")
...
(def entity-specs
  [{:entity "Shot"        :plural "shots"        :id-prefix "generalf_sho"
    :fields [:number :configuration :status :executedAt] :required [:number :configuration]
    :coerce {:number :int}            :refs {}}
   {:entity "Diagnostic"  :plural "diagnostics"  :id-prefix "generalf_dia"
    :fields [:shotId :instrument :channel :dataRef] :required [:instrument :channel]
    :coerce {}                        :refs {:shotId "Shot"}}
   {:entity "Coil"        :plural "coils"        :id-prefix "generalf_coi"
    :fields [:name :currentKa :status]         :required [:name :currentKa]
    :coerce {:currentKa :float}       :refs {}}
   {:entity "PlasmaState" :plural "plasmastates" :id-prefix "generalf_pla"
    :fields [:shotId :temperatureKev :densityM3 :betaN] :required [:temperatureKev :densityM3]
    :coerce {:temperatureKev :float :densityM3 :float :betaN :float} :refs {:shotId "Shot"}}
   {:entity "Pulse"       :plural "pulses"       :id-prefix "generalf_pul"
    :fields [:shotId :durationMs :energyMj]    :required [:durationMs :energyMj]
    :coerce {:durationMs :int :energyMj :float} :refs {:shotId "Shot"}}
   {:entity "Sample"      :plural "samples"      :id-prefix "generalf_sam"
    :fields [:shotId :parameter :value :takenAt] :required [:parameter :value]
    :coerce {:value :float}           :refs {:shotId "Shot"}}])
...
(defn emit-facts
  "EAVT facts for one record: {\"general_fusion.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "general_fusion-compat" :tier tier :entities entities} 200])
...
(defn start! [] :general_fusion-compat/ready)
